...
test("GET /api/games liste les jeux d'échelle en tête, avec leurs modes", async () => {
  const app = makeApp();
  const res = await request(app).get("/api/games");

  assert.equal(res.status, 200);
  // Les quatre jeux d'échelle ouvrent l'arcade ; les autres genres suivent.
  assert.deepEqual(
    res.body.games.slice(0, 4).map((g: any) => g.id),
